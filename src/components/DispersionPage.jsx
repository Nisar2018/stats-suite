...
import {
  ...
  getDispersionSectionTopicIds,
} from '../data/dispersionMenuConfig';
import { Sidebar } from './Sidebar';
import { DispersionContentArea } from './DispersionContentArea';

const defaultExpanded = {
  range: true,
  'quartile-deviation': false,
  'mean-deviation': false,
  variance: false,
  'standard-deviation': false,
};

export function DispersionPage({ initialTopic = 'range-ungrouped', onTopicChange }) {
  ...
  const handleSelectTopic = (topicId) => {
    ...
    const section = dispersionMenuSections.find((s) =>
  ...
  return (
    <div className="flex min-h-0 flex-1 bg-gradient-to-br from-academic-100/80 via-academic-50 to-blue-50/40">
      <Sidebar
        sections={dispersionMenuSections}
        ...
        moduleSubtitle="Dispersion"
        sectionLabel="Measurement of Dispersion"
        getSectionTopicIds={getDispersionSectionTopicIds}
      />
      ...
      <div className="flex min-h-0 min-w-0 flex-1 flex-col">
        <div className="sticky top-0 z-10 flex items-center gap-3 border-b border-academic-200 bg-white/95 px-3 py-3 shadow-sm backdrop-blur sm:px-4 md:hidden">
          ...
          <span className="min-w-0 truncate font-semibold text-blue-900">Measurement of Dispersion</span>
        </div>

        <DispersionContentArea activeTopic={activeTopic} />
      </div>
    </div>
  );
}
