...
import { addCumulativeFrequency, createId, formatNum } from '../../utils/representationStatistics';
...
export function CumulativeFrequencyContent() {
  const [rows, setRows] = useState(defaultRows);

  const result = useMemo(
    () => addCumulativeFrequency(rows.map((r) => ({ name: r.label, frequency: r.frequency }))),
    [rows],
  );
  ...
  return (
    <section className="space-y-6">
      <p className="text-academic-600">
        Cumulative frequency (C.F) and cumulative relative frequency (C.R.F) are obtained using
        the formulas below.
      </p>

      <FormulaBox label="Formulas">
        <span>C.F = sum of frequencies up to current class</span>
        <span className="mx-2">|</span>
        <span>C.R.F = (sum of preceding frequencies) / (total frequency)</span>
      </FormulaBox>
        ...
        <table className="w-full border-collapse text-sm">
          <thead>
            <tr className="bg-academic-700 text-white">
              ...
              <th className="border border-academic-600 px-3 py-2.5 text-center">C.F</th>
              <th className="border border-academic-600 px-3 py-2.5 text-center">C.R.F</th>
              <th className="w-12 border border-academic-600" />
            </tr>
          </thead>
          <tbody>
            {rows.map((row, i) => {
              const enriched = result?.rows.find((r) => r.name === row.label);
              return (
                <tr key={row.id} className={i % 2 === 0 ? 'bg-white' : 'bg-academic-50'}>
                  ...
                  <td className="border border-academic-200 px-3 py-2 text-center">
                    {enriched ? formatNum(enriched.relativeFrequency, 4) : '—'}
                  </td>
                  <td className="border border-academic-200 px-3 py-2 text-center font-medium">
                    {enriched?.cumulativeFrequency ?? '—'}
                  </td>
                  <td className="border border-academic-200 px-3 py-2 text-center font-medium">
